...
class Application():
    ...
    def run(self):
        ...
        if user_input == 1:
            ...
            input_customer_name = input(' \nPlease enter the new order you would like to create... \nWhat is the customer name? ')
            ...
            input_item_id = int(input(" \nPlease enter item id:"))
            print('\nThe item is present in the following orders: ')
            repository = OrderRepository(self.connection).find_by_item(input_item_id)
        elif user_input == 6:
            input_order_id =int(input(" \nPlease enter order id:"))
            print(f'\nThe following items were included in order {input_order_id}: ')
